# Build an Alexa Skill that access Yale API using ASK Python SDK
...
5.  **Click on "Author from scratch".**  We will configure our Lambda function next.
    ...
    2. From the "Runtime" dropdown select the python version your system supports.  This sample code works with either Python 2.7 or 3.6. To check the python version, try the following command in a terminal
        ```
        $ python --version
        Python 3.6.8
        ```
...
7.  **Finish configuring your function**. Click on your function's name (you'll find it in the middle) and scroll to the bottom of the page, you'll see a Cloud9 code editor.
    ...
    2. open [lambda_function.py](../lambda/py/lambda_function.py), look for constant:
        ```
        PASSWORD='yale password'
        USERNAME='yale email address'
        ```
        replace the USERNAME and PASSWORD to your yale email address and password, and save the file. 

    3. On your system, navigate to the lambda folder and install the dependencies in a new folder called “skill_env” using the following command:
    
        ```
        pip install -r py/requirements.txt -t skill_env
        ```
        
    4. Copy the contents of the `lambda/py` folder into the `skill_env` folder. 
    
        ```
        cp -r py/* skill_env/
        ```
    
    5. Zip the contents of the `skill_env` folder. Remember to zip the **contents** of the folder and **NOT** the folder itself.
    6. On the AWS Lambda console, change the **code entry type** drop-down to **Upload a .ZIP file**, upload the zip created in the previous step and click on **Save**.
...
8. **As a final step, copy the ARN value from the top right corner of the screen.** You will need this value in the next section of this guide.
